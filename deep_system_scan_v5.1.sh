#!/bin/bash
#===============================================================================
# DEEP SYSTEM SCAN v5.1 - Безопасная диагностика Linux для ИИ-анализа
# ТОЛЬКО ЧТЕНИЕ: Никаких изменений в системе
# Версия 5.1: Полный реестр драйверов, глубокая диагностика железа,
#             кросс-дистрибутивность, расширенные запреты, AI-parse маркеры
#===============================================================================
set -euo pipefail
...
safe_cmd() {
    ...
    if command -v timeout &>/dev/null; then
        timeout 10 bash -c "$cmd" 2>/dev/null || true
    else
...
show_scan_menu() {
    cat << 'EOF'
╔══════════════════════════════════════════════════════════════╗
║         DEEP SYSTEM SCAN v5.1 - Выбор уровня диагностики     ║
╠══════════════════════════════════════════════════════════════╣
...
print_raw_logs() {
    if [[ -n "$1" ]]; then
        ...
        if [[ $line_count -gt 500 ]]; then
            echo "[TRUNCATED] Showing last 50 of $line_count lines:"
            echo "$1" | tail -50
        else
            echo "$1" | tail -50
        fi
...
scan_drivers_firmware() {
    ...
    # Полная информация о каждом модуле: версия, описание, лицензия
    echo "• DATA: FULL_MODULE_INFO"
    for mod in $(awk '{print $1}' /proc/modules); do
        local ver desc license
        ver="$(modinfo "$mod" 2>/dev/null | grep -m1 '^version:' | cut -d: -f2 | xargs || echo "unknown")"
        desc="$(modinfo "$mod" 2>/dev/null | grep -m1 '^description:' | cut -d: -f2 | xargs || echo "N/A")"
        license="$(modinfo "$mod" 2>/dev/null | grep -m1 '^license:' | cut -d: -f2 | xargs || echo "unknown")"
        echo "  MODULE: $mod | VER: ${ver:-unknown} | DESC: ${desc:-N/A} | LICENSE: ${license:-unknown}"
    done | sort
    ...
    if check_tool lspci; then
        ...
        # Версии GPU драйверов через sysfs
        print_subsection "GPU_DRIVER_VERSIONS_SYSFS"
        for card in /sys/class/drm/card*/device/driver/module/version; do
            if [[ -f "$card" ]]; then
                local gpu_ver
                gpu_ver="$(cat "$card" 2>/dev/null || echo "N/A")"
                print_data "GPU driver version: $gpu_ver (from $card)"
            fi
        done
    else
    ...
    # Прошивка UEFI/контроллеров через fwupdmgr
    print_subsection "UEFI_FIRMWARE_FWUPD"
    if check_tool fwupdmgr; then
        local fw_devices
        fw_devices="$(safe_sudo_cmd 'fwupdmgr get-devices' 2>/dev/null | head -30)"
        if [[ "$fw_devices" != "[NEEDS_ROOT]" && -n "$fw_devices" ]]; then
            print_data "FWUPD devices detected:"
            print_raw_logs "$fw_devices"
        else
            print_status "SKIPPED [NEEDS_ROOT or no devices]"
        fi
    else
        print_status "SKIPPED [TOOL_MISSING: fwupd]"
    fi
}
...
scan_hardware_health() {
    ...
    # RAM / DIMM статус через dmidecode
    print_subsection "RAM_HARDWARE_INFO"
    if check_tool dmidecode; then
        local ram_info
        ram_info="$(safe_sudo_cmd 'dmidecode -t memory | grep -E "Size:|Type:|Speed:|Manufacturer:|Serial:|Part:"' 2>/dev/null)"
        if [[ "$ram_info" != "[NEEDS_ROOT]" && -n "$ram_info" ]]; then
            print_data "RAM hardware info:"
            print_raw_logs "$ram_info"
        else
    ...
    print_subsection "SMART_EXTENDED"
    for disk in $(lsblk -dpno NAME 2>/dev/null | grep -E '^/dev/(sd|nvme|hd)' || true); do
        local smart_err
        smart_err="$(safe_sudo_cmd "smartctl -l error $disk" 2>/dev/null | tail -10)"
        if [[ -n "$smart_err" && "$smart_err" != "[NEEDS_ROOT]" ]]; then
            print_data "$disk errors: ${smart_err:-none}"
        else
            print_data "$disk errors: [NEEDS_ROOT or no errors]"
        fi
    done
    ...
    if check_tool smartctl; then
        local disks
        disks="$(lsblk -dpno NAME 2>/dev/null | grep -E '^/dev/(sd|nvme|hd)' || true)"
        local disk_issues=""
        for disk in $disks; do
            ...
            if [[ "$smart_data" != "[NEEDS_ROOT]" && -n "$smart_data" ]]; then
                ...
                if [[ "${reallocated:-0}" -gt 0 ]] 2>/dev/null; then
                    risk="[DISK_RISK]"
                ...
                if [[ "${udma:-0}" -gt 0 ]] 2>/dev/null; then
                    risk="[DISK_RISK]"
    ...
    # NVMe health через nvme-cli
    print_subsection "NVME_HEALTH"
    if check_tool nvme; then
        local nvme_drives
        nvme_drives="$(ls /dev/nvme* 2>/dev/null | grep -v 'n' || true)"
        for drive in $nvme_drives; do
            local nvme_smart nvme_err_log
            nvme_smart="$(safe_sudo_cmd "nvme smart-log $drive" 2>/dev/null | head -20)"
            if [[ "$nvme_smart" != "[NEEDS_ROOT]" && -n "$nvme_smart" ]]; then
                print_data "$drive SMART:"
                print_raw_logs "$nvme_smart"
            fi
            nvme_err_log="$(safe_sudo_cmd "nvme error-log $drive" 2>/dev/null | head -15)"
            if [[ "$nvme_err_log" != "[NEEDS_ROOT]" && -n "$nvme_err_log" ]]; then
                print_data "$drive error-log:"
                print_raw_logs "$nvme_err_log"
            fi
        done
    else
    ...
    # Температуры через lm-sensors
    print_subsection "LM_SENSORS_THERMAL"
    if check_tool sensors; then
        local sensors_out
        sensors_out="$(sensors 2>/dev/null | grep -E 'temp|fan|voltage|in' | head -25)"
        if [[ -n "$sensors_out" ]]; then
            print_data "Thermal sensors:"
            print_raw_logs "$sensors_out"
        else
            print_status "SKIPPED [No sensor data]"
        fi
    else
        print_status "SKIPPED [TOOL_MISSING: lm-sensors]"
    fi
...
#-------------------------------------------------------------------------------
# СЕТЕВАЯ ДИАГНОСТИКА И ИНТЕРФЕЙСЫ (уровень 2+)
#-------------------------------------------------------------------------------
scan_network_interfaces() {
    print_section_header "NETWORK_INTERFACES"
    
    print_subsection "NETWORK_INTERFACES_STATS"
    ip -s link 2>/dev/null | grep -E '^[0-9]:|RX|TX|errors|dropped|overruns|carrier' | head -20 || print_status "SKIPPED [ip command failed]"
    
    print_subsection "DNS_ROUTING_TEST"
    local dns_result route_result
    dns_result="$(timeout 3 nslookup google.com 2>/dev/null | grep 'Address:' | head -1 || echo "FAILED")"
    route_result="$(ip route show default 2>/dev/null | head -1 || echo "NONE")"
    print_data "DNS resolve: $dns_result"
    print_data "Default route: $route_result"
    
    print_subsection "INTERFACE_DETAILS"
    if check_tool ip; then
        local ip_addr
        ip_addr="$(ip addr 2>/dev/null | grep -E '^[0-9]+:|inet |inet6 |state ' | head -30)"
        if [[ -n "$ip_addr" ]]; then
            print_data "Interface addresses:"
            print_raw_logs "$ip_addr"
        fi
    fi
}
...
scan_services_packages() {
    ...
    # Анализ медленных юнитов загрузки
    print_subsection "SYSTEMD_CRITICAL_CHAIN"
    if check_tool systemd-analyze; then
        local critical_chain
        critical_chain="$(safe_sudo_cmd 'systemd-analyze critical-chain' 2>/dev/null | head -20)"
        if [[ "$critical_chain" != "[NEEDS_ROOT]" && -n "$critical_chain" ]]; then
            print_data "Slow boot units:"
            print_raw_logs "$critical_chain"
        else
            print_status "SKIPPED [NEEDS_ROOT]"
        fi
    else
...
scan_logs_analysis() {
    ...
    # Journalctl предупреждения и алерты
    print_subsection "JOURNALCTL_WARNINGS_ALERTS"
    if check_tool journalctl; then
        local warn_alert
        warn_alert="$(journalctl -p warning..alert --no-pager 2>/dev/null | grep -iE 'hardware|firmware|error|fail' | tail -20)"
        if [[ -n "$warn_alert" ]]; then
            print_data "Warning/Alert logs:"
            print_raw_logs "$warn_alert"
        else
            print_data "No significant warnings/alerts in journal"
        fi
    fi
...
scan_security() {
    ...
    print_subsection "OPEN_PORTS_LISTENING"
...
#-------------------------------------------------------------------------------
# ФАЙЛОВЫЕ СИСТЕМЫ И РАЗДЕЛЫ (уровень 2+)
#-------------------------------------------------------------------------------
scan_filesystems() {
    print_section_header "FILESYSTEMS_MOUNTS"
    
    print_subsection "FS_AND_MOUNTS_DETAIL"
    df -hT 2>/dev/null | grep -v tmpfs || print_status "SKIPPED [df failed]"
    echo "--- Inodes ---"
    df -i 2>/dev/null | grep -v tmpfs || print_status "SKIPPED [df -i failed]"
    echo "--- Mount options ---"
    mount | grep -E 'ext4|xfs|btrfs|zfs' | head -10
}
...
run_scan() {
    {
        # Маркер для автоматического разбора ИИ
        echo "## AI_PARSE_RULES: Все разделы начинаются с ## [NAME], данные с • DATA: / • ISSUES_FOUND: / • STATUS:"
        echo ""
        echo "#==============================================================================="
        ...
        # Уровень 2+: Драйверы, железо, сеть, службы, логи, пользователи, ФС
